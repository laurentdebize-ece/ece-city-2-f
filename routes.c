...
void dessinerRoutes(Case cases[NB_LIGNES_MAX][NB_COLONNES_MAX], ALLEGRO_FONT *text, ALLEGRO_FONT *textBold,
                    ...
                    ALLEGRO_BITMAP *route, ALLEGRO_BITMAP *caserne, ALLEGRO_BITMAP *virageHaut, ALLEGRO_BITMAP* routeCote, ALLEGRO_BITMAP* routeDroite, ALLEGRO_BITMAP* virageBas, ALLEGRO_BITMAP* croisementQuatres, ALLEGRO_BITMAP* croisementTBas, ALLEGRO_BITMAP* croisementTHaut){
    assert (al_init());
                    ...
                    for (int j = 0; j < NB_COLONNES_MAX; ++j) {
                        if (isInRect(event.mouse.x, event.mouse.y, cases[i][j].x + 1, cases[i][j].y + 1,
                            ...
                            if (cases[i][j].occupe == 0){
                                al_draw_scaled_bitmap(routeDroite,529,337,209,118,cases[i][j].x, cases[i][j].y, 20,20,0);
                            ...
                            if (cases[i][j-1].occupe == 1 && cases[i][j+1].occupe == 0 && cases[i-1][j].occupe == 1 && cases[i+1][j].occupe == 0) {
                            ...
                            if (cases[i][j-1].occupe == 0 && cases[i+1][j].occupe == 0 && cases[i-1][j].occupe == 1 && cases[i][j+1].occupe == 1) {
                            ...
                            if (cases[i][j-1].occupe == 1 && cases[i][j+1].occupe == 1 && cases[i+1][j].occupe == 1 && cases[i-1][j].occupe == 1) {
                                al_draw_scaled_bitmap(croisementQuatres,529,337,209,118,cases[i][j].x, cases[i][j].y, 20,20,0);
                                // revoir code bitmap axe h et w
                                cases[i][j].occupe = 1;
                            }
                            // si toutes cases autour est occupée alors faire croisement 4
                            if (cases[i][j-1].occupe == 1 && cases[i][j+1].occupe == 0 && cases[i+1][j].occupe == 1 && cases[i-1][j].occupe == 1) {
                                al_draw_scaled_bitmap(croisementTBas,529,337,209,118,cases[i][j].x, cases[i][j].y, 20,20,0);
                                // revoir code bitmap axe h et w
                                cases[i][j].occupe = 1;
                            }
                            // croisements vers le bas si route à droite à gauche et en bas occupées
                            if (cases[i][j-1].occupe == 0 && cases[i][j+1].occupe == 1 && cases[i+1][j].occupe == 1 && cases[i-1][j].occupe == 1) {
                                al_draw_scaled_bitmap(croisementTHaut,529,337,209,118,cases[i][j].x, cases[i][j].y, 20,20,0);
                                // revoir code bitmap axe h et w
                                cases[i][j].occupe = 1;
                            }
